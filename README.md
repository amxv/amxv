...
- **[procoder](https://github.com/amxv/procoder)** · `Go`: A CLI that lets ChatGPT make real commits inside its locked-down coding sandbox: `prepare` creates a sanitized, offline-capable Git export, and `apply` verifies an incremental `git bundle` before updating the dedicated local task branch.
