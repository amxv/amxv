...
- **[Zodex](https://github.com/amxv/zodex)** · `Rust` · [docs](https://zodex.ashray.xyz): An MCP coding harness that puts ChatGPT directly on your Mac with your real shell, files, tools, and credentials, plus Liveboard and durable history; the same interface can run on an isolated remote Linux Sprite.
...
- **[Fidelius](https://github.com/amxv/fidelius)** · `Go` `Swift` · [docs](https://fidelius.ashray.xyz): A human-in-the-loop secrets bridge for agents that opens a native macOS or Linux prompt, then hands the agent private, short-lived files without ever printing secret values.
- **[cargo-warm](https://github.com/amxv/cargo-warm)** · `Rust` · [docs](https://cargowarm.ashray.xyz): A Rust worktree accelerator that forks useful Cargo and rustc state from a warm checkout into each new worktree's own private writable cache, keeping first compiler feedback fast without sharing one mutable target directory.
